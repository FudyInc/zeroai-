    ...
    <div className="space-y-5">
      ...
      <Card className="overflow-hidden overflow-x-auto p-0">
        <table className="w-full text-sm min-w-[640px]">
          <thead className="bg-zinc-50 dark:bg-zinc-900/30 text-zinc-500 dark:text-zinc-400 text-left text-xs uppercase tracking-wide border-b border-champagne/30">
            <tr>{['Empresa', 'Cargo', 'Contacto', 'Score', 'Etapa'].map((h) => <th key={h} className="px-4 py-3 font-semibold">{h}</th>)}</tr>
          </thead>
          <tbody>
            {isLoading && [0, 1, 2, 3, 4].map((i) => (
              <tr key={i} className="border-t border-champagne/20">
                {[0, 1, 2, 3, 4].map((j) => <td key={j} className="px-4 py-3"><Skeleton className="h-4 w-24" /></td>)}
              </tr>
            ))}

            {!isLoading && !error && rows.map((r) => (
              <motion.tr key={r.key} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }}
                onClick={() => openLead(r.key)} className="border-t border-champagne/20 hover:bg-champagne/5 dark:hover:bg-zinc-800/20 cursor-pointer transition-colors">
                <td className="px-4 py-3 font-medium">{r.company}</td>
                <td className="px-4 py-3 text-zinc-500 dark:text-zinc-400">{r.role || '—'}</td>
                <td className="px-4 py-3 text-zinc-500 dark:text-zinc-400">{r.email || r.phone || '—'}</td>
                <td className="px-4 py-3 font-extrabold tabular-nums" style={{ color: scoreColor(r.score) }}>{r.score ?? '—'}</td>
                <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                  <select value={r.stage} onChange={(e) => move(r.key, e.target.value)}
                    className="text-xs border border-zinc-200 dark:border-zinc-700/50 rounded-lg px-2.5 py-1.5 bg-white dark:bg-zinc-900/20 text-zinc-600 dark:text-zinc-300 outline-none focus:ring-2 focus:ring-champagne/40 transition">
            ...
            {!isLoading && error && (
              <tr><td colSpan={5} className=”px-4 py-12 text-center”>
                <p className=”text-rose-600 dark:text-rose-400 font-medium”>No se pudieron cargar los leads.</p>
