      ...
      <motion.div className="flex items-start justify-between gap-4 flex-wrap" variants={surface}>
        <Card className="p-5 inline-flex items-center gap-4 bg-brand text-white">
          ...
          <div>
            ...
            <div className="text-[11px] text-champagne/70 mt-1">{accounts.length} cliente{accounts.length !== 1 ? 's' : ''}</div>
          </div>
        </Card>
              ...
              <Card interactive className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <SectionTitle className="capitalize text-base">{a.client}</SectionTitle>
                    <div className="text-xs text-zinc-500 mt-1">{a.leads_per_mo ? `${a.leads_per_mo} leads/mes` : 'leads a medida'}</div>
                  </div>
                  <div className="text-sm font-extrabold text-gold-deep tabular-nums shrink-0">{a.price_clp ? clp(a.price_clp) : 'Custom'}</div>
                </div>

                <div className="mt-4 space-y-3">
                  <div>
                    <label className="block text-[11px] font-semibold uppercase tracking-wide text-pewter mb-2">Plan</label>
                    <Select value={a.tier} onChange={(e) => changePlan(a.client, e.target.value)} className="w-full text-sm">
                  ...
                  <button onClick={() => { setClient(a.client); nav('/') }}
                    className="w-full text-sm text-gold-deep font-medium flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg hover:bg-champagne/10 transition">
