      ...
      <div className="brain-workspace">
          ...
          <div className="brain-map">
            ...
            {AGENTS.map(({ name, label, role, icon: Icon, x, y }) => {
              ...
              return <button key={name} className={`brain-node ${selected === name ? 'is-selected' : ''} ${active.has(name) ? 'is-active' : ''}`} style={{ '--x': `${x}%`, '--y': `${y}%` }} onClick={() => select(name)} aria-pressed={selected === name} aria-label={`${label}: ${role}`}>
                <span className="brain-node-top"><span className="brain-node-icon"><Icon size={17} /></span><span className="brain-node-count">{statsByAgent.get(name)?.corridas ?? 0} <Activity size={10} /></span></span><strong>{label}</strong><span className="brain-node-engine" title={engineLabel(entry?.engine)}>{engineLabel(entry?.engine)}</span><small>{active.has(name) ? '● Ejecución recibida' : selected === name ? 'Seleccionado' : 'Explorar agente ↗'}</small>
              </button>
            })}
          </div>
        ...
        <aside className="brain-detail" aria-label="Detalle del agente seleccionado">
          ...
          <motion.div key={selected} initial={{ opacity: 0, y: reduced ? 0 : 5 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .18 }}>
            ...
            <div className="brain-route"><span><Cpu size={14} /> ZERO asigna la tarea</span><b>↓</b><span>{agent.label} procesa</span><b>↕</b><span title={latest?.engine}><BrainCircuit size={14} /> {engineLabel(latest?.engine || stats?.engines?.[0])}</span></div>
